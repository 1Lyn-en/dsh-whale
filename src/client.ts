...
// ── 模块级纯函数（不依赖 factory 闭包状态，提升以减小闭包体积） ──
...
window.__ModuleLoader__.load({
  ...
  factory: (require: any) => {
    ...
    /** 各模式的估算节省率（用于反推正常输出量，与 README 宣传值一致） */
     ...
     */
    function WhaleSvgIcon({ size = 38, className = '' }: any) {
    ...
    function showWhaleTransition(onComplete?: () => void) {
      if (document.querySelector('.whale-transition-overlay')) {
    ...
    const DERIVED_TOKENS = Object.freeze({
      '--dsw-alias-label-primary-foreground': (palette: any) =>
        palette === PALETTES.light ? '#FFFFFF' : '#061024',
      ...
      '--dsw-alias-state-warn-tertiary': (palette: any) =>
        `color-mix(in srgb, ${palette.marker} 18%, ${palette.surface1})`,
      '--dsw-alias-state-success-tertiary': (palette: any) =>
        `color-mix(in srgb, ${palette.success} 18%, ${palette.surface1})`,
    });
...
html.dsh-theme-cyberui-loaded div:has(> div > [data-slot='sidebar']) { background: transparent !important; }
...
html.dsh-theme-cyberui-loaded [data-slot='sidebar'] > div {
  ...
  --dsw-specific-sidebar-fill: var(--whale-sidebar-fill, rgb(2 7 22 / 96%)) !important;
  position: relative;
  color: #ECF7FF;
...
html.dsh-theme-cyberui-loaded [data-slot='sidebar'] > div {
  background: var(--cyberui-sidebar);
  overflow: hidden;
}
...
`;
    ...
    function buildOverrides(sidebarFill = 'rgb(2 7 22 / 96%)') {
      const tokens = {};
      for (const [token, role] of Object.entries(TOKEN_ROLES))
        (tokens as any)[token] = { light: PALETTES.light[role], dark: PALETTES.dark[role] };
      for (const [token, resolve] of Object.entries(DERIVED_TOKENS))
        (tokens as any)[token] = { light: resolve(PALETTES.light), dark: resolve(PALETTES.dark) };
      Object.assign(tokens, {
        '--dsw-specific-sidebar-fill': { light: sidebarFill, dark: sidebarFill },
      });
      return tokens;
    }
    ...
    function overrideCyberuiTokens(sidebarFill?: string) {
      if (!themeCtx) return;
      const previousDispose = tokenDispose;
      tokenDispose = themeCtx.theme.overrideTokens(OVERRIDE_SOURCE, buildOverrides(sidebarFill));
      previousDispose?.();
    }

    function applyCyberuiTheme(_ctx: any): () => void {
      const root = document.documentElement;
      const originalTitle = document.title;
      root.classList.add(ROOT_CLASS);
      overrideCyberuiTokens();
    ...
    const THEME_STORAGE_KEY = 'whale-theme-config';
    const BACKGROUND_DB_NAME = 'dsh-whale-theme';
    const BACKGROUND_DB_VERSION = 1;
    const BACKGROUND_STORE_NAME = 'theme-assets';
    const BACKGROUND_KEY = 'custom-background';
    const MAX_BACKGROUND_BYTES = 12 * 1024 * 1024;
    const MAX_BACKGROUND_DIMENSION = 3840;
    const BACKGROUND_WEBP_QUALITY = 0.9;
    ...
    interface ThemePreset {
      ...
      neon: string; // 主色
      violet: string; // 辅色
      sidebarRgb: string; // 侧边栏底色 "r g b"
      scrimRgb: string; // 遮罩底色 "r g b"
      glowRgb: string; // 发光主色 "r g b"
      glowVioletRgb: string;
    }
    ...
    const THEMES: Record<string, ThemePreset> = {
      ...
      abyss: {
        ...
        scrimRgb: '1 3 12',
        glowRgb: '14 165 233',
        glowVioletRgb: '6 182 212',
      },
      matcha: {
        name: '🍵 Matcha',
        neon: '#84CC16',
        violet: '#22C55E',
        sidebarRgb: '245 247 240',
        scrimRgb: '240 244 232',
        glowRgb: '132 204 22',
        glowVioletRgb: '34 197 94',
      },
      sunset: {
        name: '🌅 Sunset',
        neon: '#F97316',
    ...
    interface ThemeConfig {
      ...
      sidebarOpacity: number; // 0-1，界面透明度（1 = 完全透明）
    ...
    let legacyBackgroundDataUrl: string | null = null;
    let backgroundObjectUrl: string | null = null;

    function clampConfigNumber(value: unknown, fallback: number, max = 1): number {
    ...
    function loadConfig(): ThemeConfig {
      try {
        ...
        legacyBackgroundDataUrl =
          typeof parsed.bgImage === 'string' && parsed.bgImage.startsWith('data:image/')
            ? parsed.bgImage
            : null;
        return {
          preset:
            typeof parsed.preset === 'string' && THEMES[parsed.preset]
              ? parsed.preset
              : DEFAULT_CONFIG.preset,
          scrimOpacity: clampConfigNumber(parsed.scrimOpacity, DEFAULT_CONFIG.scrimOpacity),
          sidebarOpacity: clampConfigNumber(
            parsed.sidebarOpacity,
            DEFAULT_CONFIG.sidebarOpacity,
            0.95,
          ),
          glowIntensity: clampConfigNumber(parsed.glowIntensity, DEFAULT_CONFIG.glowIntensity),
        };
      } catch {
    ...
    function openBackgroundDb(): Promise<any> {
      return new Promise((resolve, reject) => {
        ...
        request.onblocked = () => reject(new Error('背景图片存储被其他页面占用'));
      });
    }

    async function withBackgroundStore<T>(
      ...
      try {
        return await new Promise<T>((resolve, reject) => {
          ...
          transaction.onabort = () =>
            reject(transaction.error || new Error('背景图片存储事务失败'));
          transaction.onerror = () =>
            reject(transaction.error || new Error('背景图片存储事务失败'));
        });
      } finally {
    ...
    function getStoredBackground(): Promise<any | undefined> {
      return withBackgroundStore<any | undefined>('readonly', (store) => store.get(BACKGROUND_KEY));
    }

    function storeBackground(blob: any): Promise<void> {
      return withBackgroundStore<void>('readwrite', (store) => store.put(blob, BACKGROUND_KEY));
    }

    function deleteStoredBackground(): Promise<void> {
      return withBackgroundStore<void>('readwrite', (store) => store.delete(BACKGROUND_KEY));
    }

    function dataUrlToBlob(dataUrl: string): Promise<any> {
      return window.fetch(dataUrl).then((response: any) => {
        if (!response.ok) throw new Error('旧背景图片迁移失败');
        return response.blob();
      });
    }

    function setRuntimeBackground(blob: any | null) {
      if (backgroundObjectUrl) {
        window.URL.revokeObjectURL(backgroundObjectUrl);
        backgroundObjectUrl = null;
      }
      if (blob) backgroundObjectUrl = window.URL.createObjectURL(blob);
    }

    // ── 应用配置到 CSS 变量（核心：只做 setProperty） ────────────────
    let rafId = 0;
    function applyConfig(cfg: ThemeConfig) {
      // 用 rAF 节流，滑块拖动时合并多次写入
      if (rafId) cancelAnimationFrame(rafId);
      rafId = requestAnimationFrame(() => {
        const theme = THEMES[cfg.preset] || THEMES['cyber-neon'];
        const root = document.documentElement;

        root.style.setProperty('--cyberui-neon', theme.neon);
        root.style.setProperty('--cyberui-violet', theme.violet);

        // 侧边栏：透明度越高，主题色越淡，直接透出与页面对齐的背景图。
        const sidebarTransparency = Math.min(0.95, Math.max(0, cfg.sidebarOpacity));
        const sidebarAlpha = 1 - sidebarTransparency;
        const sidebarTint = `rgb(${theme.sidebarRgb} / ${sidebarAlpha})`;
        root.style.setProperty('--cyberui-sidebar', sidebarTint);
        root.style.setProperty('--whale-sidebar-fill', sidebarTint);
        if (themeDispose) overrideCyberuiTokens(sidebarTint);

        // scrim 遮罩
        const so = cfg.scrimOpacity;
        root.style.setProperty(
          '--cyberui-scrim',
          `linear-gradient(90deg, rgb(${theme.scrimRgb} / ${so}) 0%, rgb(${theme.scrimRgb} / ${so * 0.55}) 48%, rgb(${theme.scrimRgb} / ${so * 0.75}) 100%)`,
        );

        // 激活项渐变
        root.style.setProperty(
          '--cyberui-active',
          `linear-gradient(90deg, ${theme.neon}2e, ${theme.violet}26)`,
        );

        // 背景发光层颜色
        root.style.setProperty(
          '--whale-glow-a',
          `rgb(${theme.glowRgb} / ${cfg.glowIntensity * 0.14})`,
        );
        root.style.setProperty(
          '--whale-glow-b',
          `rgb(${theme.glowVioletRgb} / ${cfg.glowIntensity * 0.08})`,
        );

        // 背景图
        if (backgroundObjectUrl) {
          root.style.setProperty('--cyberui-art', `url("${backgroundObjectUrl}")`);
        } else {
          root.style.removeProperty('--cyberui-art');
        }
      });
    }
    ...
    function injectOverrideStyles() {
      ...
      styleInjected = true;
      const style = document.createElement('style');
      style.id = 'whale-theme-overrides';
      style.textContent = `
    /* 背景三层叠加：scrim + 发光 + 背景图 */
    html.dsh-theme-cyberui-loaded body {
      background-image:
        var(--cyberui-scrim),
        radial-gradient(ellipse 55% 45% at 50% 42%, var(--whale-glow-a) 0%, var(--whale-glow-b) 40%, transparent 72%),
        var(--cyberui-art);
    }

    /* workspaces 区域：透明背景 + 禁用底部淡出遮罩/mask，消除白色渐变条 */
    html.dsh-theme-cyberui-loaded [data-slot='sidebar.workspaces'] {
      --dsw-specific-sidebar-fill: var(--whale-sidebar-fill, rgb(2 7 22 / 96%)) !important;
      background: transparent !important;
      -webkit-mask: none !important;
      mask: none !important;
    }
    html.dsh-theme-cyberui-loaded :has(> [data-slot='sidebar.workspaces'])::after,
    html.dsh-theme-cyberui-loaded :has(> [data-slot='sidebar.workspaces'])::before {
      display: none !important;
      background: transparent !important;
    }
    /* 原生滚动提示是一个真实的 CSS-module span；半透明叠色会形成深色矩形。 */
    html.dsh-theme-cyberui-loaded [data-slot='sidebar.workspaces'] span[class$='_fade'] {
      display: none !important;
    ...
    .whale-theme-panel {
      ...
      color: #ECF7FF; font-family: system-ui, sans-serif; font-size: 14px;
      display: none;
    }
    .whale-theme-panel.open { display: block; }
    .whale-theme-panel h3 { margin: 0 0 14px; font-size: 16px; font-weight: 600; color: #24D8FF; }
    .whale-theme-panel .section { margin-bottom: 16px; }
    .whale-theme-panel .label { display: block; margin-bottom: 8px; color: #AFC2E4; font-size: 13px; }
    .whale-theme-panel .theme-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; }
    .whale-theme-panel .theme-btn {
      padding: 8px 6px; border: 1px solid rgb(255 255 255 / 10%);
      background: rgb(255 255 255 / 4%); border-radius: 8px;
      color: #ECF7FF; cursor: pointer; font-size: 12px;
      transition: all 0.15s;
    }
    .whale-theme-panel .theme-btn:hover { background: rgb(36 216 255 / 12%); border-color: rgb(36 216 255 / 30%); }
    .whale-theme-panel .theme-btn.active { background: rgb(36 216 255 / 18%); border-color: #24D8FF; }
    .whale-theme-panel input[type="range"] {
      width: 100%; height: 6px; -webkit-appearance: none; appearance: none;
      background: rgb(255 255 255 / 12%); border-radius: 3px; outline: none;
    }
    .whale-theme-panel input[type="range"]::-webkit-slider-thumb {
      -webkit-appearance: none; width: 16px; height: 16px; border-radius: 50%;
      background: #24D8FF; cursor: pointer; box-shadow: 0 0 8px rgb(36 216 255 / 50%);
    }
    .whale-theme-panel .row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; }
    .whale-theme-panel .val { color: #24D8FF; font-size: 12px; font-variant-numeric: tabular-nums; }
    .whale-theme-panel .btn-row { display: flex; gap: 10px; }
    .whale-theme-panel .action-btn {
      flex: 1; padding: 9px; border: 1px solid rgb(255 255 255 / 12%);
      background: rgb(255 255 255 / 5%); border-radius: 8px;
      color: #ECF7FF; cursor: pointer; font-size: 13px; transition: all 0.15s;
    }
    .whale-theme-panel .action-btn:hover { background: rgb(36 216 255 / 12%); }
    .whale-theme-panel .status {
      min-height: 18px; margin-top: 10px; color: #8FE7FF; font-size: 12px;
    }
    .whale-theme-panel .status.error { color: #FF9B9B; }
    .whale-theme-panel .close-btn {
      position: absolute; top: 14px; right: 16px; background: none; border: none;
      color: #7E96C2; cursor: pointer; font-size: 18px; line-height: 1;
    }
    .whale-theme-panel .close-btn:hover { color: #ECF7FF; }
    .whale-theme-gear-btn {
      position: relative; display: inline-flex; align-items: center; justify-content: center;
      width: 36px; height: 36px; border: none; background: transparent;
      cursor: pointer; border-radius: 8px; color: #7E96C2; transition: all 0.2s;
    }
    .whale-theme-gear-btn:hover { background: rgb(36 216 255 / 10%); color: #24D8FF; }
  `;
      document.head.appendChild(style);
    }
    ...
    function compressImageSource(source: any): Promise<any> {
      return new Promise((resolve, reject) => {
        const inputUrl = window.URL.createObjectURL(source);
        const img = new Image();
        const cleanup = () => window.URL.revokeObjectURL(inputUrl);
        img.onload = () => {
          try {
            ...
            if (width > MAX_BACKGROUND_DIMENSION || height > MAX_BACKGROUND_DIMENSION) {
              const ratio = Math.min(
                MAX_BACKGROUND_DIMENSION / width,
                MAX_BACKGROUND_DIMENSION / height,
              );
            ...
            canvas.toBlob(
              (blob: any) => {
                ...
                if (!blob || blob.type !== 'image/webp') {
                  reject(new Error('浏览器不支持 WebP 背景压缩'));
                  return;
                }
                resolve(blob);
              },
        ...
        img.onerror = () => {
          cleanup();
          reject(new Error('图片内容损坏或浏览器不支持该格式'));
        };
        img.src = inputUrl;
      });
    }

    function compressImage(file: File): Promise<any> {
    ...
    let backgroundStorageError = '';

    function setPanelStatus(message: string, error = false) {
    ...
    function persistConfig() {
      try {
        ...
        setPanelStatus('设置保存失败，请检查浏览器存储权限', true);
        return false;
      }
    }

    function buildPanel(): any {
      const p = document.createElement('div');
      p.className = 'whale-theme-panel';
      p.innerHTML = `
    ...
    <div class="section">
      ...
      <div class="theme-grid">
        ${Object.entries(THEMES)
          .map(
            ([id, t]) =>
              `<button class="theme-btn${id === config.preset ? ' active' : ''}" data-preset="${id}">${t.name}</button>`,
          )
          .join('')}
      </div>
    </div>
    <div class="section">
      <div class="row"><span class="label">遮罩浓度</span><span class="val" id="whale-val-scrim">${Math.round(config.scrimOpacity * 100)}%</span></div>
      <input type="range" min="0" max="100" value="${Math.round(config.scrimOpacity * 100)}" data-slider="scrimOpacity">
    </div>
    <div class="section">
      <div class="row"><span class="label">侧边栏透明度</span><span class="val" id="whale-val-sidebar">${Math.round(config.sidebarOpacity * 100)}%</span></div>
      <input type="range" min="0" max="95" value="${Math.round(config.sidebarOpacity * 100)}" data-slider="sidebarOpacity">
    </div>
    <div class="section">
      <div class="row"><span class="label">鲸鱼发光</span><span class="val" id="whale-val-glow">${Math.round(config.glowIntensity * 100)}%</span></div>
      <input type="range" min="0" max="100" value="${Math.round(config.glowIntensity * 100)}" data-slider="glowIntensity">
    </div>
    <div class="section">
      <span class="label">背景图</span>
      <div class="btn-row">
        <button class="action-btn" data-action="upload">上传背景</button>
        <button class="action-btn" data-action="reset-bg">恢复默认</button>
      </div>
      <div class="status" data-role="status" role="status" aria-live="polite"></div>
      <input type="file" accept="image/png,image/jpeg,image/webp,image/avif" id="whale-bg-input" style="display:none">
    </div>
  `;
      ...
      p.addEventListener('click', (e: any) => {
        ...
        if (preset) {
          ...
          p.querySelectorAll('.theme-btn').forEach((b: any) =>
        ...
        if (action === 'upload') {
          p.querySelector('#whale-bg-input')?.click();
        }
        if (action === 'reset-bg') {
          setPanelStatus('正在恢复默认背景...');
          void deleteStoredBackground()
            .then(() => {
              setRuntimeBackground(null);
              legacyBackgroundDataUrl = null;
              const saved = persistConfig();
              applyConfig(config);
              if (saved) {
                backgroundStorageError = '';
                setPanelStatus('已恢复默认背景');
              }
            })
            .catch(() => setPanelStatus('默认背景恢复失败，请检查浏览器存储权限', true));
        }
      });

      // 滑块
      p.querySelectorAll('input[type="range"]').forEach((slider: any) => {
        slider.addEventListener('input', () => {
          const key = (slider as any).dataset.slider as keyof ThemeConfig;
          const val = parseInt((slider as any).value, 10) / 100;
          (config as any)[key] = val;
          const valEl = p.querySelector(
            `#whale-val-${key.replace('Opacity', '').replace('Intensity', '')}`,
          );
      ...
      // 文件上传
      p.querySelector('#whale-bg-input')?.addEventListener('change', async (e: any) => {
        const file = (e.target as any).files?.[0];
        if (!file) return;
        (e.target as any).value = '';
        setPanelStatus('正在压缩并保存背景...');
        try {
          const blob = await compressImage(file);
          await storeBackground(blob);
          setRuntimeBackground(blob);
          legacyBackgroundDataUrl = null;
          const saved = persistConfig();
          applyConfig(config);
          if (saved) {
            backgroundStorageError = '';
            setPanelStatus('背景已保存为 WebP');
          }
        } catch (error) {
          const message = error instanceof Error ? error.message : '背景图上传失败';
    ...
    function ensureGearButton() {
      // 查找鲸鱼按钮的父容器
      const whaleBtn = document.querySelector('.whale-header-btn');
      if (!whaleBtn || whaleBtn.parentElement?.querySelector('.whale-theme-gear-btn')) return;

      const gear = document.createElement('button');
      gear.type = 'button';
      gear.className = 'whale-theme-gear-btn';
      gear.title = '鲸鱼主题设置';
      ...
      gear.addEventListener('click', (e: any) => {
        ...
        if (!panel) {
          panel = buildPanel();
          if (backgroundStorageError) setPanelStatus(backgroundStorageError, true);
        }
        panel.classList.toggle('open');
      });
      whaleBtn.parentElement?.insertBefore(gear, whaleBtn.nextSibling);
    }

    // ── 对外接口 ─────────────────────────────────────────────────────
    function removeWhiteFade() {
      ...
      for (let i = 0; i < els.length; i++) {
        ...
        if (el.dataset.whaleFadeHidden) continue;
        const cs = getComputedStyle(el);
        const bg = cs.backgroundImage || '';
        const hasWhiteGradient =
        ...
        if (hasWhiteGradient) {
          ...
          if (r.height <= 30 && r.bottom >= sRect.bottom - 40 && r.width > 100) {
            el.style.setProperty('display', 'none', 'important');
            el.dataset.whaleFadeHidden = '1';
          }
        }
      }
    }

    async function restoreRuntimeBackground() {
      try {
        let blob = await getStoredBackground();
        if (!blob && legacyBackgroundDataUrl) {
          const legacyBlob = await dataUrlToBlob(legacyBackgroundDataUrl);
          blob = await compressImageSource(legacyBlob);
          await storeBackground(blob);
        }
        if (legacyBackgroundDataUrl) {
          saveConfig(config);
          legacyBackgroundDataUrl = null;
        }
        setRuntimeBackground(blob || null);
        backgroundStorageError = '';
      } catch {
        backgroundStorageError = '自定义背景读取失败，请检查浏览器存储权限';
        if (legacyBackgroundDataUrl) {
          try {
            setRuntimeBackground(await dataUrlToBlob(legacyBackgroundDataUrl));
          } catch {
            setRuntimeBackground(null);
          }
        }
    ...
    async function initThemeCustomizer() {
      config = loadConfig();
      injectOverrideStyles();
      applyConfig(config);
      await restoreRuntimeBackground();
      applyConfig(config);

      // 移除侧边栏底部白色淡出渐变（DSH 原生 DOM 元素）
      removeWhiteFade();
      const fadeObs = new MutationObserver(() => {
        requestAnimationFrame(removeWhiteFade);
      });
      fadeObs.observe(document.body, { childList: true, subtree: true });
      setTimeout(() => fadeObs.disconnect(), 8000);

      // 鲸鱼按钮是 React 渲染的，可能稍后才出现，用 MutationObserver 等待
      if (document.querySelector('.whale-header-btn')) {
        ensureGearButton();
      } else {
      ...
      document.addEventListener('click', (e: any) => {
        ...
        if (
          !panel.contains(e.target as any) &&
          !(e.target as any).closest('.whale-theme-gear-btn')
        ) {
    ...
    let customizerInited = false;

    function activateCyberuiTheme() {
      if (themeDispose || !themeCtx) return;
      themeDispose = applyCyberuiTheme(themeCtx);
      if (!customizerInited) {
        customizerInited = true;
        void initThemeCustomizer().catch(() => {
          backgroundStorageError = '鲸鱼主题设置初始化失败';
        });
      }
    }
